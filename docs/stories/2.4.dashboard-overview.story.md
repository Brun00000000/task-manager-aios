...
- [x] API /api/dashboard/stats retorna DashboardStats corretamente
- [x] Grafico renderiza sem erros no cliente
- [x] Refetch automatico a cada 60s funcionando (verificar no Network tab)
...
- [x] **Task 1: Instalar recharts** (AC: 5)
  - [x] 1.1 `npm install recharts`

- [x] **Task 2: getDashboardStats no TaskRepository** (AC: 1, 2, 3, 4, 5)
  - [x] 2.1 Query contagens: total, todo, in_progress, done (WHERE deleted_at IS NULL)
  - [x] 2.2 Query overdue: due_date < hoje AND status != 'done' AND deleted_at IS NULL
  - [x] 2.3 Query due_today: due_date = hoje AND status != 'done' AND deleted_at IS NULL (retornar TaskSummary[])
  - [x] 2.4 Query upcoming: proximas 5 tarefas por due_date ASC (retornar TaskSummary[])
  - [x] 2.5 Query activity: COUNT de tarefas concluidas por dia nos ultimos 7 dias

- [x] **Task 3: API Route** (AC: 1-5)
  - [x] 3.1 Criar `src/app/api/dashboard/stats/route.ts`
  - [x] 3.2 GET: autenticar, chamar getDashboardStats, retornar DashboardStats
  - [x] 3.3 Retornar estado vazio (zeros) se usuario nao tem tarefas

- [x] **Task 4: useDashboardStats hook** (AC: 6)
  - [x] 4.1 Criar `src/features/dashboard/hooks/useDashboardStats.ts`
  - [x] 4.2 useQuery com queryKey: ['dashboard', 'stats']
  - [x] 4.3 refetchInterval: 60 * 1000 (60 segundos)

- [x] **Task 5: StatsCard** (AC: 1, 2, 7)
  - [x] 5.1 Criar `StatsCard.tsx` — props: title, value, icon, variant (default/red)
  - [x] 5.2 Card "Atrasadas": variant red (borda e texto vermelhos)
  - [x] 5.3 Estado vazio: todos os cards com valor 0 + CTA "Criar primeira tarefa"

- [x] **Task 6: ActivityChart** (AC: 5)
  - [x] 6.1 Criar `ActivityChart.tsx` como Client Component
  - [x] 6.2 BarChart do recharts: eixo X = dia da semana, eixo Y = tarefas concluidas
  - [x] 6.3 Usar dynamic import (next/dynamic ssr:false) se necessario para evitar SSR issues

- [x] **Task 7: UpcomingList** (AC: 3, 4)
  - [x] 7.1 Criar `UpcomingList.tsx` — duas secoes: "Vencendo hoje" e "Proximas"
  - [x] 7.2 Cada item: titulo + prazo formatado (date-fns) + badge prioridade
  - [x] 7.3 Link para /tasks ao clicar em cada item

- [x] **Task 8: DashboardEmpty** (AC: 7)
  - [x] 8.1 Criar `DashboardEmpty.tsx` — mensagem + botao "Criar sua primeira tarefa" (link /tasks)
  - [x] 8.2 Exibir quando total === 0

- [x] **Task 9: Pagina /dashboard** (AC: 1-7)
  - [x] 9.1 Atualizar `src/app/(dashboard)/dashboard/page.tsx` (substituir placeholder)
  - [x] 9.2 Layout: grid de 5 StatsCards + ActivityChart + UpcomingList
  - [x] 9.3 Loading skeleton durante fetch inicial

- [x] **Task 10: Verificacao final** (AC: 1-7)
  - [x] 10.1 `npm run lint && npm run typecheck && npm run test` passando
  - [x] 10.2 Build limpo
...
| 2026-02-21 | 1.1 | Status Draft para Ready — GO (10/10). Métricas e interface do DashboardStats alinhadas com arquitetura. Risco SSR do recharts documentado. Pronta para implementação. | @po (Pax) |
