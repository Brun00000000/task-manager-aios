...
- [ ] Busca full-text funciona (testar com palavras parciais)
- [ ] URL atualiza ao aplicar filtros e recupera ao recarregar pagina
- [ ] Debounce de 300ms verificado (Network tab: query nao dispara a cada tecla)
...
- [ ] **Task 1: Habilitar pg_trgm no Supabase** (AC: 4)
  - [ ] 1.1 Acessar Supabase Dashboard > SQL Editor
  - [ ] 1.2 Executar: `CREATE EXTENSION IF NOT EXISTS pg_trgm;`
  - [ ] 1.3 Criar indice: `CREATE INDEX IF NOT EXISTS tasks_title_trgm_idx ON tasks USING gin(title gin_trgm_ops);`
  - [ ] 1.4 Criar indice: `CREATE INDEX IF NOT EXISTS tasks_desc_trgm_idx ON tasks USING gin(description gin_trgm_ops);`
  - [ ] 1.5 Registrar SQL em `supabase/migrations/` para versionamento

- [ ] **Task 2: useDebounce hook** (AC: 5)
  - [ ] 2.1 Criar `src/hooks/useDebounce.ts` — hook simples com useState + useEffect + setTimeout
  - [ ] 2.2 Parametros: value, delay (default 300ms)

- [ ] **Task 3: useTaskFilters hook** (AC: 1, 2, 3, 6, 7)
  - [ ] 3.1 Criar `src/features/tasks/hooks/useTaskFilters.ts`
  - [ ] 3.2 Ler filtros de useSearchParams (status[], priority[], category_id[], due, search)
  - [ ] 3.3 Funcoes: setFilter, removeFilter, clearFilters — atualizam URL com router.push
  - [ ] 3.4 Computar hasActiveFilters (para mostrar/esconder "Limpar filtros")
  - [ ] 3.5 Computar activeFilterChips para renderizar chips removiveis

- [ ] **Task 4: Atualizar TaskRepository.list()** (AC: 4, 7)
  - [ ] 4.1 Adicionar suporte a parametro `search` na query
  - [ ] 4.2 Full-text search: `.or(\`title.ilike.%${search}%,description.ilike.%${search}%\`)` (ou ilike + pg_trgm)
  - [ ] 4.3 Garantir todos os filtros existentes (status, priority, category_id, due) funcionam combinados

- [ ] **Task 5: FilterBar** (AC: 1, 4, 5)
  - [ ] 5.1 Criar `FilterBar.tsx` — layout horizontal com todos os filtros
  - [ ] 5.2 Campo de busca (Input) com icone Search do Lucide
  - [ ] 5.3 Debounce na busca: usar useDebounce antes de atualizar URL
  - [ ] 5.4 Multi-select de Status: checkboxes para todo, in_progress, done
  - [ ] 5.5 Multi-select de Prioridade: checkboxes para low, medium, high, urgent
  - [ ] 5.6 Multi-select de Categoria: usar dados do useCategories
  - [ ] 5.7 Select de Prazo: hoje, esta semana, atrasadas, sem prazo

- [ ] **Task 6: FilterChips** (AC: 2, 3)
  - [ ] 6.1 Criar `FilterChips.tsx` — renderiza chips para cada filtro ativo
  - [ ] 6.2 Chip com label legivel (ex: "Status: A fazer") + botao X para remover
  - [ ] 6.3 Botao "Limpar filtros" ao lado dos chips (visivel quando hasActiveFilters)

- [ ] **Task 7: Contagem de resultados** (AC: 8)
  - [ ] 7.1 API GET /api/tasks ja retorna `meta.total` — usar para contagem total
  - [ ] 7.2 Exibir "Exibindo N de M tarefas" abaixo da FilterBar
  - [ ] 7.3 Ocultar quando nao ha filtros e resultado = total

- [ ] **Task 8: Atualizar pagina /tasks** (AC: 1-8)
  - [ ] 8.1 Adicionar FilterBar acima da lista
  - [ ] 8.2 Adicionar FilterChips entre barra e lista
  - [ ] 8.3 Passar filtros do useTaskFilters para useTasks
  - [ ] 8.4 Envolver em Suspense (necessario para useSearchParams no App Router)
  - [ ] 8.5 Exibir contagem de resultados
