...
- [x] Drag funciona em desktop (mouse) e mobile (touch)
- [x] Posicao persistida ao recarregar pagina
- [x] Lista de 50+ itens sem lag durante drag
...
- [x] **Task 1: Instalar dependencias dnd-kit** (AC: 1)
  - [x] 1.1 `npm install @dnd-kit/core @dnd-kit/sortable @dnd-kit/utilities`

- [x] **Task 2: Reorder no TaskRepository** (AC: 4)
  - [x] 2.1 Adicionar metodo `reorder(userId, items: {id: string, position: number}[])` ao TaskRepository
  - [x] 2.2 Implementar bulk UPDATE: UPDATE tasks SET position = $2 WHERE id = $1 AND user_id = $3
  - [x] 2.3 Usar Promise.all para updates paralelos (ou upsert em batch via Supabase)

- [x] **Task 3: API Route reorder** (AC: 4)
  - [x] 3.1 Criar `src/app/api/tasks/reorder/route.ts`
  - [x] 3.2 PATCH: receber `{ items: [{id, position}] }`, validar com Zod, chamar TaskRepository.reorder()
  - [x] 3.3 Retornar 204 (No Content) em sucesso

- [x] **Task 4: useReorderTasks hook** (AC: 4)
  - [x] 4.1 Criar `src/features/tasks/hooks/useReorderTasks.ts`
  - [x] 4.2 useMutation PATCH /api/tasks/reorder
  - [x] 4.3 Optimistic update via estado local em SortableTaskList
  - [x] 4.4 onError: rollback para estado anterior (snapshot)

- [x] **Task 5: SortableTaskCard** (AC: 2, 3, 5)
  - [x] 5.1 Criar `SortableTaskCard.tsx` como Client Component
  - [x] 5.2 Usar `useSortable({ id: task.id })` do @dnd-kit/sortable
  - [x] 5.3 DragHandle: icone GripVertical do Lucide, visivel no hover (group-hover/sortable)
  - [x] 5.4 Aplicar `transform` e `transition` do useSortable para animacao
  - [x] 5.5 Estilo durante drag: `opacity-50`
  - [x] 5.6 Reutilizar TaskCard internamente (composicao)

- [x] **Task 6: SortableTaskList** (AC: 1, 3, 7)
  - [x] 6.1 Criar `SortableTaskList.tsx` como Client Component
  - [x] 6.2 DndContext com sensores: PointerSensor (distance: 8px) + TouchSensor (delay: 250ms)
  - [x] 6.3 SortableContext com items=[task.id] e strategy=verticalListSortingStrategy
  - [x] 6.4 onDragEnd: usar arrayMove do @dnd-kit/sortable para reordenar array local
  - [x] 6.5 Apos arrayMove: posicoes com offset de paginacao ((offset + i + 1) * 1000)
  - [x] 6.6 DragOverlay: renderizar TaskCard com rotate-1 scale-105 durante drag

- [x] **Task 7: Atualizar pagina /tasks** (AC: 1-7)
  - [x] 7.1 Substituir TaskList por SortableTaskList em TasksContent.tsx
  - [x] 7.2 Filtros (Story 2.5) continuam funcionando com SortableTaskList
  - [x] 7.3 Query de tasks ordenada por `position ASC` (ja implementado no TaskRepository)

- [x] **Task 8: Verificacao final** (AC: 1-7)
  - [x] 8.1 `npm run lint && npm run typecheck && npm run test` passando
  - [x] 8.2 Build limpo
  - [x] 8.3 Testar em mobile: drag touch funciona sem disparar scroll
  - [x] 8.4 File List completo abaixo
...
| 2026-02-21 | 1.1 | Status Draft para Ready — GO (10/10). dnd-kit setup e position calc documentados, sensores touch/pointer configurados. Risco SSR endereçado. Pronta para implementação. | @po (Pax) |
| 2026-02-21 | 1.2 | Implementação completa. lint ✅ typecheck ✅ tests 8/8 ✅ build ✅. Optimistic update via estado local + rollback por snapshot. Position offset por pagina para evitar colisão. Status → Done. | @dev (Dex) |
...
**Modificados:**
- src/lib/repositories/TaskRepository.ts — método reorder() com Promise.all ✅
- src/features/tasks/components/TasksContent.tsx — substituído TaskList por SortableTaskList ✅
